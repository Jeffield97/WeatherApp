...
const SearchBar = ({ setLat, setLon, getWeather }) => {
  const searchBycity = async (cityName) => {
    try {
      const res = await fetch(
        `http://api.openweathermap.org/geo/1.0/direct?q=${cityName}&limit=5&appid=00ce3cbe5164b0438313a3f261362d8a`,
      );
  ...
  return (
    <form className="form-control w-full max-w-xs mt-12 mx-auto" onSubmit={handleForm}>
      ...
      />
      <button className="btn btn-outline mt-5">Get info</button>
    </form>
  );
};
